...
public:
     ...
     * @return std::vector<uint8_t> Hash output
     */
    static std::vector<uint8_t> hash(std::span<const uint8_t> data, size_t output_size = HASH_SIZE);
     ...
     * @return std::string Hexadecimal representation of the hash
     */
    static std::string hashHex(std::span<const uint8_t> data, size_t output_size = HASH_SIZE);
