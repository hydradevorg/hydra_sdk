...
int GetCommand::execute(const std::vector<std::string>& args) {
    ...
    std::string output_file = get_param_value(args, "--output", "");
    ...
    // If no output file is specified, use the basename of the file_path
    if (output_file.empty()) {
        size_t last_slash = file_path.find_last_of('/');
        if (last_slash != std::string::npos) {
            output_file = file_path.substr(last_slash + 1);
        } else {
    ...
    try {
        ...
        while (true) {
            ...
            out_file.write(reinterpret_cast<const char*>(buffer.data()), bytes_read);
        }
        ...
        std::cout << "Successfully extracted file to: " << output_file << std::endl;
        return 0;
    } catch (const std::exception& e) {
...
void GetCommand::print_help() const {
    std::cout << "Usage: hydra vfs get [OPTIONS] CONTAINER_PATH FILE_PATH" << std::endl;
    std::cout << "Extract a file from a container to the local filesystem" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "  --output FILE         Output file path (default: basename of FILE_PATH)" << std::endl;
